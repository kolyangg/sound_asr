...
class SortaGradDataLoader:
    def __init__(
        self, dataset, batch_size, collate_fn, num_workers, pin_memory, 
        num_epochs_sorted=1, drop_last=True, shuffle=True, **kwargs
    ):
        self.dataset = dataset
        self.batch_size = batch_size
        self.collate_fn = collate_fn
        ...
        print("Precomputing spectrogram lengths...")
        self.spectrogram_lengths = [
            self.compute_length(data["spectrogram"]) for data in dataset
        ]
        print("Precomputation done.")

    @staticmethod
    def compute_length(spectrogram, threshold=1e-3):
        energy = torch.sum(spectrogram, dim=0)
        non_zero = energy > threshold
        if non_zero.any():
    ...
    def __iter__(self):
        if self.current_epoch < self.num_epochs_sorted:
            # Sort by precomputed spectrogram lengths
            sorted_indices = sorted(
                range(len(self.dataset)),
                key=lambda i: self.spectrogram_lengths[i]
            )
            sampler = SubsetRandomSampler(sorted_indices)
            print(f"Epoch {self.current_epoch + 1}: Sorting dataset by length.")
        else:
            # Use random sampling after initial epochs
            sampler = RandomSampler(self.dataset)
            print(f"Epoch {self.current_epoch + 1}: Shuffling dataset randomly.")
