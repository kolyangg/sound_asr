...
class CTCTextEncoder:
    ...
    def encode(self, text: str) -> torch.Tensor:
        ...
        """
        # print(f"[DEBUG] encode(text='{text}') => use_bpe={self.use_bpe}")
        if self.use_bpe and self.sp is not None:
            ...
            # print(f"   -> normalized='{normalized_text}', token_indices={token_indices}")
    ...
    def decode_simple(self, indices: List[int]) -> str:
        ...
        """
        # print(f"[DEBUG] decode_simple => indices={indices}, use_bpe={self.use_bpe}")
        if self.use_bpe and self.sp is not None:
            collapsed = []
            prev_idx = None
            for idx in indices:
    ...
    def decode(self, indices: List[int]) -> str:
        ...
        """
        # print(f"[DEBUG] decode => len(indices)={len(indices)}")
        if self.decoder:
    ...
    def decode_logits(self, logits: Union[torch.Tensor, List[List[float]], np.ndarray]) -> str:
        ...
        """
        # print("[DEBUG] decode_logits => start")
    ...
    def decode_indices(self, indices: Union[torch.Tensor, List[int], np.ndarray]) -> str:
        ...
        """
        # print(f"[DEBUG] decode_indices => {indices}")
    ...
    def ctc_decode(self, logits: Union[torch.Tensor, List[int], np.ndarray]) -> str:
        ...
        # print(f"[DEBUG] ctc_decode => predicted_indices={predicted_indices}")
