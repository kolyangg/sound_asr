...
class CTCTextEncoder:
    def __init__(self, alphabet=None, arpa_path=None, binary_path=None, unigram_path=None,
        ...
        if use_bpe:
            ...
            # Add the blank token to the tokenizer's vocabulary
            ...
            # Retrieve tokenizer's tokens excluding other special tokens
            special_tokens = set(self.tokenizer.all_special_tokens)
            self.bpe_tokens = [tok for tok in self.tokenizer.get_vocab().keys() if tok not in special_tokens]
    ...
    def encode(self, text) -> torch.Tensor:
        ...
        if self.use_bpe:
            try:
                # Tokenize the text into BPE tokens
    ...
    def decode(self, indices) -> str:
        ...
        if self.use_bpe:
            ...
            valid_indices = [
                ...
                if int(idx) != self.blank_index and 0 < int(idx) < len(self.ind2char)
            ]
            try:
            ...
            return "".join([self.ind2char[int(ind)] for ind in indices if int(ind) != self.blank_index])
    ...
    def ctc_decode(self, logits: torch.Tensor) -> str:
        ...
        for t in range(predictions.size(0)):
            ...
            current_char = self.ind2char.get(token_idx, self.BLANK_TOKEN)
        ...
        if self.use_bpe:
            ...
            text = self.tokenizer.convert_tokens_to_string(decoded)
